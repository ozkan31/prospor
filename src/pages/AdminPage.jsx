...
const formatAddress = (address) => {
  ...
  const parts = [
    ...
    address.phone,
    [address.cityName, address.districtName, address.neighborhoodName].filter(Boolean).join(" / "),
    address.line
  ].filter(Boolean);
  return parts.join(" • ");
};
...
export default function AdminPage() {
    ...
    const loadAdminData = async () => {
      ...
      try {
        const [productsJson, ordersJson, usersJson] = await Promise.all([
          apiRequest("/admin/products", {}, adminToken),
          apiRequest("/admin/orders", {}, adminToken),
          apiRequest("/admin/users", {}, adminToken)
        ]);
        ...
        if (!mounted) return;
        setAdminDataError(error.message || "Admin verileri yüklenemedi.");
      } finally {
        if (mounted) setAdminDataLoading(false);
      }
    };
  ...
  const startEditingProduct = (product) => {
    ...
    setCategoryItems(Array.isArray(product.category) ? product.category : []);
    setColorItems(Array.isArray(product.colors) ? product.colors : []);
    setSizeItems(Array.isArray(product.sizes) ? product.sizes.map(String) : []);
  };
  ...
  const handleSaveProduct = async (e) => {
    ...
    try {
      ...
      await apiRequest(path, { method, body: formData }, adminToken);
      await loadProducts();
      resetProductForm();
      setIsAddOpen(false);
      setSaveMessage(editingProductId ? "Ürün başarıyla güncellendi." : "Ürün başarıyla eklendi.");
    } catch (error) {
      setSaveError(error.message || (editingProductId ? "Ürün güncellenemedi." : "Ürün eklenemedi."));
    } finally {
    ...
    return (
      <div className="container page-pad auth-wrap">
        ...
        <form className="card auth-card" onSubmit={handleAdminLogin}>
          <input type="email" placeholder="Admin e-posta" value={loginForm.email} onChange={(e) => setLoginForm((prev) => ({ ...prev, email: e.target.value }))} />
          <input type="password" placeholder="Şifre" value={loginForm.password} onChange={(e) => setLoginForm((prev) => ({ ...prev, password: e.target.value }))} />
  ...
  return (
    <div className="container page-pad admin-page">
      ...
      {adminDataError && <p className="form-error">{adminDataError}</p>}
      ...
      <div className="admin-layout">
        ...
        <section className="admin-content">
          {adminDataLoading && <p className="muted">Admin verileri yükleniyor...</p>}

          {tab === "products" && (
            <div>
              <button className="primary-btn" onClick={() => setIsAddOpen((prev) => !prev)}>
                {isAddOpen ? "Ürün Formunu Kapat" : "Ürün Ekle"}
              </button>

              {isAddOpen && (
                <form className="admin-add-form" onSubmit={handleSaveProduct}>
                  <div className="inline-actions">
                    <strong>{editingProductId ? "Ürün Düzenle" : "Yeni Ürün"}</strong>
                    {editingProductId && (
                  ...
                  <div className="form-grid">
                    <input placeholder="Ürün kimliği (opsiyonel)" value={newProduct.id} disabled={Boolean(editingProductId)} onChange={(e) => setNewProduct((p) => ({ ...p, id: e.target.value }))} />
                    ...
                    <div className="admin-tag-input-row">
                      <input className="admin-half-input" placeholder="Kategori ekle" value={categoryInput} onChange={(e) => setCategoryInput(e.target.value)} />
                      <button type="button" className="secondary-btn" onClick={() => { setCategoryItems((prev) => addUniqueItem(prev, categoryInput)); setCategoryInput(""); }}>
                        Ekle
                      </button>
                    </div>
                    ...
                    <div className="admin-tag-input-row">
                      <input className="admin-half-input" placeholder="Renk ekle" value={colorInput} onChange={(e) => setColorInput(e.target.value)} />
                      <button type="button" className="secondary-btn" onClick={() => { setColorItems((prev) => addUniqueItem(prev, colorInput)); setColorInput(""); }}>
                        Ekle
                      </button>
                    </div>
                    <div className="admin-tag-list">
                    ...
                    <div className="admin-tag-input-row">
                      <input className="admin-half-input" placeholder="Numara ekle" value={sizeInput} onChange={(e) => setSizeInput(e.target.value)} />
                      <button type="button" className="secondary-btn" onClick={() => { setSizeItems((prev) => addUniqueItem(prev, sizeInput)); setSizeInput(""); }}>
                        Ekle
                      </button>
                    </div>
                    <div className="admin-tag-list">
                    ...
                    <textarea className="full" placeholder="Ek galeri URL (virgülle veya satır satır, opsiyonel)" value={newProduct.gallery} onChange={(e) => setNewProduct((p) => ({ ...p, gallery: e.target.value }))} />
                  </div>

                  <input ref={fileInputRef} type="file" accept="image/*" multiple style={{ display: "none" }} onChange={handleFileSelect} />

                  <div className="admin-image-grid">
                    <button type="button" className="admin-image-picker" onClick={openFilePicker} aria-label="Görsel seç">+</button>
                  ...
                  <div className="inline-actions">
                    <button className="primary-btn" type="submit" disabled={saveLoading}>
                      {saveLoading ? "Kaydediliyor..." : editingProductId ? "Ürünü Güncelle" : "Ürünü Kaydet"}
                    </button>
                  </div>
                </form>
              )}
              ...
              <div className="admin-table-wrap">
                <table className="admin-table">
                  <thead>
                    <tr><th>Ürün</th><th>Marka</th><th>Fiyat</th><th>Stok</th><th>Kategori</th><th>İşlem</th></tr>
                  </thead>
                  <tbody>
                    {products.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td><button className="secondary-btn" type="button" onClick={() => startEditingProduct(p)}>Düzenle</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {tab === "orders" && (
            <div className="admin-orders">
              {orders.length === 0 && !adminDataLoading && <p className="muted">Henüz sipariş yok.</p>}
              {orders.map((order) => (
                <article className="admin-order-card" key={order.id}>
                  ...
                  <div className="admin-order-meta">
                    <p><strong>Müşteri:</strong> {order.user?.name || "-"} | {order.user?.phone || "-"} | {order.user?.email || "-"}</p>
                    <p><strong>Adres:</strong> {formatAddress(order.address)}</p>
                    <p><strong>Kargo:</strong> {formatPrice(order.shipping)}</p>
                  </div>
                  <div className="admin-table-wrap">
                    <table className="admin-table">
                      <thead>
                        <tr><th>Ürün</th><th>Varyant</th><th>Adet</th><th>Birim Fiyat</th></tr>
                      </thead>
                      <tbody>
                        {order.items.map((item, index) => (
                          <tr key={`${order.id}-${item.productId || item.name}-${index}`}>
                            <td>{item.name}</td>
          ...
          {tab === "users" && (
            <div className="admin-table-wrap">
              <table className="admin-table">
                <thead>
                  <tr><th>Ad</th><th>Soyad</th><th>E-posta</th><th>Telefon</th><th>Kayıt Tarihi</th></tr>
                </thead>
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id}>
